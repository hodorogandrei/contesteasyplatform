<?php
    ...
    if($_SESSION['logat'] == 'Da')
    {
        ...
        if(isset($setid))
        {
            if($_SESSION['global']==1 ? $acces=1 : $acces=check_perm("Editare galerie"));
            if($acces!=1)
            ...
            if(!empty($_FILES['uploadedfile']['tmp_name']))
            {
                ...
                if(isset($ok) && $ok==1)
                {
                    ...
                    if(move_uploaded_file($_FILES['uploadedfile']['tmp_name'], $target_path)) 
                    {
                        ...
                        list($width, $height, $type, $attr) = getimagesize('../images/gallery/'.$link.'');
                        include('../libraries/simpleImage.php');

                        $image = new SimpleImage();
                        $image->load('../images/gallery/'.$link.'');
                        $image->resizeToHeight(197);
                        $image->save('../images/gallery/'.$link.'');

                        if(is_valid($link))
                            $link = make_safe($link);
                        $sql = "INSERT INTO `gallery`(`gfile`, `galt`) VALUES ('$link', '$galt')";
                        mysql_query($sql);
                    } 
                }
                else
        ...
        {
            $mesaj = "Imaginea a fost &#351;tears&#259;!";

            $sql = 'SELECT * FROM `gallery` WHERE id='.$_GET['sterge'].'';
            $result = mysql_query($sql);
            $rand = mysql_fetch_object($result);
            $delfile = $rand -> gfile;
            unlink('../images/gallery/'.$delfile.'');

            $sql = "DELETE FROM `gallery` WHERE id=".$_GET['sterge']."";
            mysql_query($sql);
        }

        if($_POST['delete'])
        {
            $mesaj = "Imaginile selectate au fost &#351;terse! V&#259; rug&#259;m a&#351;tepta&#355;i!";
            echo '<meta http-equiv="refresh" content="2;URL=editare-galerie.php">';
        }
    ...
    <html xmlns="http://www.w3.org/1999/xhtml">
        ...
        <body>
                        ...
                        <form action="<?php echo $_SERVER['PHP_SELF'];?>" method="post" enctype="multipart/form-data">
                            ...
                            <div class="hiddendiv">
                                <?php
                                    ...
                                    $uploader->SaveDirectory="../images/gallery/";
                                    $uploader->Render();
                                    ...
                                    if(count($files) > 0)
                                    {
                                        ...
                                        {
                                            echo("<tr>");
                                            echo("<td>");echo("<img src='libraries/phpuploader/resources/circle.png' border='0' />");echo("</td>");
                            ...
                            <noscript>
                                ...
                                Fi&#537;ier: <input type="file" name="uploadedfile" />
                                <input name="submitnojs" type="submit" value="Ad&#259;ugare imagine"/>
                            </noscript>
                        </form>
                        <br/><b>Administrare imagini:</b><br/><br/>
                        <?php
                                    ...
                                    <tbody>
                                        <?php
                                            while($rand = mysql_fetch_array($result))
                                            {
                                                ...
                                                <td class="separate"><center>'.$rand['ordine'].'</center></td>
                                                <td class="separate">'.$rand['gfile'].'</td>
                                                <td class="separate">'.$rand['galt'].'</td>
                                                <td class="separate">'.$rand['link'].'</td>
                                                <td class="separate"><center><a href="../images/gallery/'.$rand['gfile'].'" class="highslide" onclick="return hs.expand(this);"><img src="../images/gallery/'.$rand['gfile'].'" alt="'.$rand['galt'].'" width="100" height="51" /></a></center></td>
                                                <td class="separate"><a href="editare-foto.php?id='.$rand['id'].'"><img src="images/edit.png" border="0" /></a>&nbsp;<a class="delete-link" href="editare-galerie.php?sterge='.$rand['id'].'" id="'.$rand['id'].'"><img src="images/del.png" border="0" /></a></td>
                                                </tr>
                                                ';
                                            }
                                            if($_POST['delete'])
                            ...
                            <br/>Ordinea &icirc;n galerie <i>(actualizarea se va realiza automat)</i>:<br/><br/>
                            <ul id="gallery" class="hiddendiv">
                                <?php
                                    $sql = "SELECT * FROM `gallery` ORDER BY ordine ASC"; 
                                    $result = mysql_query($sql);
                                    while($rand = mysql_fetch_array($result)) 
                                    {
                                        ...
                                        echo '<div><img src="../images/gallery/'.$rand['gfile'].'" alt="'.$rand['galt'].'" width="100" height="51" /></div>';
                                        echo "\n";
                                        echo "\n</li>";
                                    }
                                ?>
                            ...
                            <?php noscript_text("Nu pute&#355;i edita dec&acirc;t cu Javascript activat!");
                                ...
                                echo "Nu exist&#259; imagini &icirc;n galerie.";?>
                    </div>
            ...
            <script type="text/javascript" src="js/editare-galerie.js"></script>
