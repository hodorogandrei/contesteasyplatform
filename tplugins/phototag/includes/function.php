...
function get_results() {
  //Query the DB
  $qry = " SELECT id, title, x1, y1, x2, y2, width, height FROM harta ";
...
if(!empty($_POST['tag'])) {
	...
	$qry = " INSERT INTO harta (id, title, x1, y1, x2, y2, width, height) " .
	...
	//Sanitize the POST values
	$id = clean($_GET['id']);
	$qry = " DELETE FROM harta where id = $id ";
	$result=mysql_query($qry);
	header("location: ../index.php");
}
