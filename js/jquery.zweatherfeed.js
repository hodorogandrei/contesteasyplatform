...
(function($){
			...
			$.ajax({
				...
				success: function(data) {
					...
					if (data.query) {
						...
						if (options.showerror) $e.html('<p>Informa&#355;iile nu sunt disponibile.</p>');
					}
				},
				error: function(data) {
					if (options.showerror)  $e.html('<p>Cererea de preluare a vremii a e&#351;uat. Verifica&#355;i conexiunea la Internet!</p>');
				}
			});
	...
	var _callback = function(e, feed, options,locationid) {
		...
		if (options.highlow) html += '<div class="weatherRange">Maxima: '+ wf.high +'&deg;<br/> Minima: '+ wf.low +'&deg;</div>';
		if (options.wind) html += '<div class="weatherWind">V&acirc;nt: '+ wd +' '+ feed.wind.speed + feed.units.speed +'</div>';
		if (options.link) {
			html += '';
		}
